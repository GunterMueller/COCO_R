...
(* Simple Taste cross reference program *)
  ...
  BEGIN
    ...
    IF ~ FileIO.Okay THEN
      FileIO.WriteString(FileIO.con, 'Could not open input file');
      FileIO.WriteLn(FileIO.con);
      FileIO.QuitExecution;
    END;
    ...
    IF ~ FileIO.Okay THEN
      FileIO.WriteString(FileIO.con, 'Could not open listing file');
      FileIO.WriteLn(FileIO.con);
      (* default Scanner.lst to screen *) lst := FileIO.con;
    END;
    ...
    FileIO.WriteString(FileIO.con, 'Parsing'); FileIO.WriteLn(FileIO.con);
    Parse; PrintListing;

    (* examine the outcome from (* Scanner.errors *) *)
    IF errors # 0
      THEN
        FileIO.WriteString(FileIO.con, 'Incorrect source');
      ELSE
        FileIO.WriteString(FileIO.con, 'Parsed correctly - see ');
        FileIO.WriteString(FileIO.con, listName);
